...
- `npm run dev` - 本地开发服务器
- `npm test` - 运行所有测试（JS + Bash）
- `npm run test:js` - 仅 JavaScript 测试
- `npm run test:bash` - 仅 Bash 测试
...
### 当前状态
项目已实现完整的自动化测试套件，**覆盖 Worker 和 Shell 脚本，100% 通过率**。
...
### 项目特点
- **双语支持**：`fuckits.sh` 是统一源码，构建时生成英文 `main.sh` 和中文 `zh_main.sh`
...
### 修改建议
1. **修改安装脚本**：
   - 编辑 `fuckits.sh`，不要直接改生成产物 `main.sh` 或 `zh_main.sh`
...
- 所有业务逻辑函数
- 语言检测和切换功能
- 构建时生成 main.sh 和 zh_main.sh

### worker.js
Cloudflare Worker 主文件（约 1764 行，含嵌入的 base64 脚本），处理：
...
- `handleRobotsTxt()` - robots.txt 端点（SEO/Agent 可发现性）
- `handleSitemap()` - sitemap.xml 端点（SEO/Agent 可发现性）
- `handleWellKnown()` - .well-known 端点（Agent 发现协议）
- `handleApiCatalog()` - API 目录端点
- `handleOAuthDiscovery()` - OAuth 发现端点
- `handleMcpServerCard()` - MCP Server Card 端点
- `handleAgentSkillsIndex()` - Agent Skills 索引端点
...
**main.sh / zh_main.sh 核心函数（构建产物，约 3217 行）**：
...
**zh_main.sh**：由同一份 `fuckits.sh` 生成，默认语言为中文，功能与 `main.sh` 保持一致。
