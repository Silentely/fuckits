...
| `/scripts` | 构建和部署脚本集合 | Bash | build.sh, deploy.sh, one-click-deploy.sh, setup.sh, common.sh | 100% (6/6) |
...
- **版本**：2.1.0
- **总文件数**：约 46 个核心文件（已扫描 44 个，覆盖率 96%）
- **代码行数**：
  - worker.js: ~690 行（含 base64 嵌入）
  - main.sh: ~2253 行
  - zh_main.sh: ~1735 行
  - scripts: ~489 行（build 82 + deploy 34 + one-click-deploy 179 + setup 104 + common 90）
