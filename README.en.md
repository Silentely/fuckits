...
<br>
<img src="./og-image.png" alt="fuckits — AI Natural Language to Shell Command" width="640">
<br>
</div>
...
You should see `status: "ok"` and `services.apiKey: true`. The response also includes `stats.totalCalls` and `stats.uniqueIPs` for daily usage statistics (note: admin bypass requests are not counted). If not, double-check the domain binding and OpenAI secrets.
